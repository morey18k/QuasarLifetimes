...
for num_stack in [10, 50, 100, 500, 1000, 5000, 10000, 50000, 100000]:
	...
	act_covar = covar[:-1, :-1]
	sign, lndet = np.linalg.slogdet(act_covar)
	print(lndet)
	continue
	covar = np.ma.array(covar, mask = (covar ==0.0))
...
#	plt.savefig('covar_tq{1}_{0}.pdf'.format(bin_size, t))
	np.save('covar_matrix_tq{1}_{0}.npy'.format(bin_size,t), covar.data)
