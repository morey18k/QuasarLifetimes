...
def like_computation(covars, x, y, models, all_pixels = True, diag = True, lower = 1190):
    ...
    if not all_pixels:
        low_pixel = np.argmin(np.abs(wave_stack - lower))
    else:
        low_pixel = 0
...
for k, stack in enumerate([15]):
    ...
    for bin_size in bin_list: 
        ...
        like_grid = np.array([log_like_grid(ws, fs, invcovs[k], flux_interpolated[k]) for k in range(len(flux_interpolated))])
        print(np.amax(like_grid))        
        ...
        ax.fill_between(ws, fs-std, fs+std, alpha = 0.25, label = "1-$\\sigma$ Uncertainty Range for $\\Sigma_{\\mathrm{data}}$", color = 'grey')
        plt.legend()
        plt.ylim(-0.1, 1.1)
        plt.xlim(lower, 1220)
